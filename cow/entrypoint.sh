...
CONFIG_FILE="${HOME}/.cow/config.rc"

if [ "$1" == "cow" ]
then
   cat > $CONFIG_FILE  <<EOF
listen = 0.0.0.0:7777
logFile =${HOME}/.cow/log
...
EOF
   ...
   if [ -f ${HOME}/.cow/$USER_PASSWORD_FILE ]
   then
       cat >> $CONFIG_FILE <<EOF
userPasswd = ${HOME}/.cow/${USER_PASSWORD}
EOF
   else
       if [ ! -z ${USER_PASSWORD} ]
       then
   ...
   [ -f ${HOME}/.cow/blokced ] || touch ${HOME}/.cow/blocked
   [ -f ${HOME}/.cow/direct ] || touch ${HOME}/.cow/direct
   [ -f ${HOME}/.cow/stat ] || touch ${HOME}/.cow/stat

   cat >> $CONFIG_FILE  <<EOF
statFile = ${HOME}/.cow/stat
